...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   - IP-адрес моего компьютера - 192.168.0.103, порт, на котороый он отправляет данные 80, его же порт равен 55883 (не до конца понял, какой порт имеется в виду, поэтому написал оба).
...
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   ...
   - 128.119.245.12
   - gaia.cs.umass.edu принимает данные на порте 80, а отправляет на порт 55883
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - 63
   - У этого пакета есть метка [SYN] (а также проставлен флаг Syn)
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - 64
   - 1
   - Это порядковый номер следующего(то бишь первого) байта, ожидаемого от отправителя
   - У него него проставлено 2 флага: Asknowledgement и Syn
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - 66. См. скрин из первого вопроса
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент
   ...
   - [66 - 71]
   - Они все были отправлены в одно время (если считать время от первого сегмета, содержащего POST, то в 0.0 секунд)
   - Для 70 и 71 ASK был получен спустя 0.122232 секунды, что и является их RTT. Для более ранних пакетов персональных ASK-ов не пришло, а потому RTT любого их перечисленных пакетов равно 0.122232 секунды
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - На передачу данных ушло
   - Последний ASK- TCP-сегмент о закрытии соединения был отправлен спустя примерно 2 секунды после открытия. Размер нашего файла равен 152138 байт, а значит скорость передачи составляет $\dfrac{152138}{2} = 76069$ байт\с или же 74 кб\с.
...
<img src="images/images/task1.png" width=600 />
...
Пусть размер окна равен $n$. Проблема возникает только если окно получателя после получения $n$ пакетов пересекается со окном отправителя на момент отправки первого из них. Будем считать, что окно отправителя перед отправкой выглядело как $[0, n)$, тогда окно получателя после получения пакетов выглядит как $[n, 2n \mod k)$. Ясно, что окна пересекутся, если $2n >= k$, а значиит максимальное $n$ равно $\dfrac{k}{2}$ с округлением вниз.
