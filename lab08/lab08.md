...
#### Решение
Так как в TCP размер окна увеличивается на 1 MSS каждые RTT секунд при успешной передаче, то для того, чтобы размер окна вырос с $\frac{W}{2}$ до $W$ требуется, чтобы прошло $T = \frac{W}{2 MSS}$ RTT. Средняя скорость передачи TCP-соединения равна $S = \frac{3 W}{4}$ байт \ RTT, а значит $W = \frac{4 S}{3}$. Подставим в $T$: $T = \frac{2 S}{3 MSS}$. Ч.Т.Д.
...
#### Решение
Во всех пунктах время на установление соедниения занимает RTT.

1. В этом пункте $\frac{3R}{R} > RTT > \frac{S}{R}$. Мы получим ASK-пакета на первый сегмент в $2RTT$. Затем отправим еще 2 пакета, получим ASK-и на них в $3RTT$ и $3RTT + \frac{S}{R}$. Далее мы будем слать пакеты непрерывно (нам будут успевать приходить ASK-и до того, как исчерпается окно), а значит последний пакет клиент получит в $3RTT + \frac{12S}{R}$

2. В данном случае схема будет чуть иной: мы начем отправлять пакеты непрерывно, погда получим ASK на 4 пакет в момент времени $4RTT$ (в этот момент в <<полете>> у нас еще пакеты 5, 6 и 7). После этого остается отправить еще 8 пакета, а потому итоговое время составит $4RTT + \frac{8S}{R}$

3. Тут я вообще мало представляю, что происходит (пакет от сервера к клиенту идет $\frac{S}{R}$, на при это ASK на него приходит за $RTT < \frac{S}{R}$ - что-то очень странное). Если имеется в виду, что ASK на первый пакет успевате прийти до того, как мы захотим отправить второй, то потратим мы $RTT + \frac{15S}{R}$.
...
#### Решение
За $n$ полученных ASK-ов мы увеличим наше окно до $\frac{W}{2} (1 + a)^n$. Так как максимальный размер окна равен $n$, то $\frac{W}{2} (1 + a)^n = W$, а значит $n = \log_{1 + a} 2$. За это время будет передано $\frac{W}{2} + \frac{W}{2} (1 + a) + \dots + \frac{W}{2} (1 + a)^n = \frac{W}{2}(1 + (1 + a) + \dots + (1 + a)^n) = \frac{W}{2} \cdot \frac{(1 + a)^{n + 1} - 1}{1 + a - 1} = \frac{W}{2} \cdot \frac{2 (1 + a) - 1}{a} = \frac{W \cdot (2a + 1)}{2a}$ сегментов. Тогда частота потерь равна $\frac{2a}{W (2a + 1)}$
...
#### Решение
Как и было сказано в условии, сначала клиент потратим $RTT_{FE}$ на установление соединения с внешним сервером, потом последний получает данные от дата-центра, с которым уже установлено скоростное соединение,     за $RTT_{BE} + \text{время обработки}$, и посылает данные клиенту еще за $3RTT_{FE}$ (по условию). Сумма выходит такой же, как и в (1).
