...
#### Решение
Во всех пунктах время на установление соедниения занимает 1 RTT.
1. В этом пункте $\frac{3R}{R} > RTT > \frac{S}{R}$. В начале $cwnd = S$, а значит будет передан один сегмент. Он будет передан за $\frac{S}{R}$, что меньше $RTT$, а значит $cwnd$ станет равен $3S$ (). После этого будет передано сразу два пакета за $\frac{2R}{S}$

2. В данном случае $RTT > \frac{3R}{S}$, а значит первые 2 передачи данных пройдут успешно

3. В этом случае даже один пакет не успевает передаться за $RTT$, а значит значение $cwnd$ будет равно $S$ все время. В этом случае время передачи будет равно $15 \frac{R}{S} + RTT$.
