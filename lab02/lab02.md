...
#### Решение
Последний пакет будет стоять в очереди источника $(P - 1) \dfrac{L}{R}$ секунд. После этого он, не простояв ни в одной очереди коммутатора, доберется до получателя за $N \dfrac {L}{R}$, а потому, итоговая задержка составит $(P - 1 + N) \dfrac{L}{R}$
...
#### Решение
Если передавать файл полностью, то зарежка составит $\dfrac{5 \cdot 2^{13}}{200} + \dfrac{5 \cdot 2^3}{3} + \dfrac{5 \cdot 2^3}{2} \approx 238.13$ секунд. Если передавать файл пакетами, задержка уменьшится.
...
#### Решение
Вычислим максимальное возможное количество одновременных пользователей: 2 Мбит/с / 100 Кб/c = 20. Тогда остается посчитать $P(12 \leq X \leq 20)$ , где $X$ - биномиальное распределение 60 пользователй с коэффицинетом 0.2, что примерно равно 0,55.
...
#### Решение
Так как размер каждого сегмента равен $S$ бит, всего их будет отправлено $C = \dfrac{X}{S}$ штук. Последний пакет простоит в очереди на отправку $(C - 1)\dfrac{L}{R}$ секунд, а после этого дойдет до получателя за $3 \dfrac{L}{R}$. секунд. Таким образом, итоговая задержка составит $(C + 2)\dfrac{L}{R} = \dfrac{(80 + S) \cdot (\frac{X}{S} + 2)}{R} = \dfrac{(80 + S)(X + 2S)}{R S}$ секунд. Найдем проиводную по $S$, чтобы найти минимум получившейся функции. Она равна $\dfrac{-80X + 2S^2}{RS^2}$ и равна нулю в точке $S = \sqrt{40X}$
...
#### Решение
1. Так как задержка передачи составляет $\dfrac{L}{R}$, то сумма задержек равна $\dfrac{I \cdot L}{R (1 – I)} + \dfrac{L}{R} = \dfrac{L}{R} \cdot \dfrac{1}{1 - I}$
2. Подставим $I = \dfrac{L a}{R}$ в полученный результат: $\dfrac{L}{R} \cdot \dfrac{1}{1 - I} = \dfrac{L}{R} \cdot \dfrac{1}{1 - a \frac{L}{R}}$. Заменим $\dfrac{L}{R}$ на переменную $x$, получим $\dfrac{x}{1 - a x}$. Эта функция от $x$ растет, пока $x \in [0, \frac{1}{2a}]$ и убывает при $x \in [\dfrac{1}{2a}, \infty)$ (так как производная равна $\dfrac{1}{(1 - ax)^2}$).
